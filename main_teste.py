...
from solve import a_star, is_goal, depth
...
def level_times_depth(file_name):
    levels = open("levels.txt", "r").read().split("\n")
    f = open(file_name, "w")
    start = time.time()
    i = 1 
    for level in levels:
        ...
        level = level.split(" ")[1]
        size = int(math.sqrt(len(level)))
        x = depth(level, is_goal, (size,size))
        print(f"SOLUTION LENGTH: { i, len(x)}")
        f.write(f"{i};{time.time()-start};{len(x)}\n")
        i += 1
...
if __name__ == "__main__":
    ...
    level_times_depth("times/depth.txt")
    ...
    level_no_depth("times/depth_no.txt")
